...
import CustomSelect from "../components/Select/CustomSelectMuiltil";
...
import { updateProjectData } from "../action/projectAction";
const Projectinfor = () => {
  //  description content
  ...
  const projectInfor = useSelector(state => state.project.projectInfor)
  const projectId = projectInfor?.id
  
  if(!projectInfor){
    return <NotFont />
  }
  
  const [name , setName] = useState(projectInfor?.name)
  const [url , setUrl] = useState(projectInfor?.url)
  const [description, setDescription] = useState(projectInfor?.description)    
  const [projectCategory , setProjectCategory] = useState(projectInfor.category)
...
const handleChangeInforOfProject = async ()=>{
  console.log(formData)
  dispatch(updateProjectData({projectId , formData}))
    ...
    <div className="flex flex-col w-[70%] h-[100vh] mx-auto">
      ...
      <div className="w-full flex flex-col gap-3">
        ...
        <div className="w-full">
          ...
          <CustomSelectOne value={projectCategory} setValue = {setProjectCategory}  options={optionsForSelectProjectCategory}  />
        </div>
        {/* button save change */}
        <div onClick={()=>{handleChangeInforOfProject()}} className="bg-blue-600 w-[120px] p-2 text-[15px] text-[#fff] rounded-md mt-3 cursor-pointer hover:bg-blue-400 ">
